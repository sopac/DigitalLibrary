...
<div id="header">
    <h1><g:message code="search_results"></g:message> : ${params.q}</a></h1>
    <g:form url='[controller: "searchable", action: "index"]' id="searchableForm" name="searchableForm" method="get">
        <g:textField name="q" value="${params.q}" size="50"/>
        <g:checkBox value="${params.textSearch}" style="margin-top: -4px; margin-left: 20px" name="textSearch"></g:checkBox>
        <span style=""><g:message code="search_publications"></g:message></span>
        <br/>
        <input class="btn-primary" type="submit" value="${message(code: 'search')}"/>
    ...
    <div class="title">
        <span>
            <g:if test="${haveQuery && haveResults}">
                ...
                <g:if test="${params.textSearch}">
                     <u>within Publication Text</u>
                </g:if>
                <br/><br/>
            </g:if>
            <g:else>
        ...
        <div class="results">
            <g:each var="result" in="${searchResult.results}" status="index">
                <div class="result">
                    ...
                    <g:set var="title" value="${result.toString()}"></g:set>


                    <g:set var="p" value="${digitallibrary.Publication.get(result.id)}"/>
                    <g:if test="${params.textSearch}">
                        <g:set var="p" value="${digitallibrary.Publication.get(digitallibrary.PublicationText.get(result.id).publicationId)}"/>
                        <g:set var="link" value="${createLink(controller: 'publication', action: 'show', id: p.id)}"/>
                        <g:set var="title" value="${p.title}"></g:set>
                    </g:if>

                    <div class="name"><a href="${link}">${title}</a></div>

                    <g:set var="desc" value="${p.author}, ${p.division.ENname},  ${p.publishedYear}, ${p.language},  ${p.category.category}"/>
                    <g:if test="${desc.size() > 120}"><g:set var="desc" value="${desc[0..120] + '...'}"/></g:if>
                    <div class="desc">${desc.encodeAsHTML()}</div>

                    <div class="displayLink">
                        <i>${Math.round((searchResult.scores[index] * 100)).toString()}%</i>
                        <br/><br/>
                    </div>
                </div>
            </g:each>
        ...
        <div>
            <div class="paging">
                <g:if test="${haveResults}">
                    ...
                    <g:else><g:paginate style="padding: 10px; text-spacing: 10px ;letter-spacing: 10px" controller="searchable" action="index" params="[q: params.q]" total="${searchResult.total}" prev="&lt; previous" next="next &gt;"/></g:else>
                </g:if>
            </div>
        </div>
    </g:if>
